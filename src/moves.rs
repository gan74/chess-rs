...
pub fn moves(board: &Board, pos: Pos) -> BitBoard {
	moves_for_piece(board, board.piece_at(pos), pos)
}

pub fn moves_for_piece(board: &Board, colored: ColoredPiece, pos: Pos) -> BitBoard {
	...
	match piece {
		...
		Piece::Queen => {
			moves_for_piece(board, Piece::Rook.colored(color), pos)
			.with_board(moves_for_piece(board, Piece::Bishop.colored(color), pos))
		}
